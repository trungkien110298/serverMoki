...
var  search_json = express.Router();
...
search_json.post("/search", function (req, res) {
    ...
    MongoClient.connect(config.database, { useNewUrlParser: true }, function (err, db) {
        ...
        dbo.collection("Sanpham").find().toArray(function (err, result) {
            ...
            res.json({code: 1000, message: "OK", data: data })// Đổi thành kiểu JSON
            db.close();
        });
    });
});

module.exports = search_json;